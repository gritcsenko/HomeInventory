    ...
    where TModule : IModule
{
    ...
    public TGiven Metrics(out IVariable<IMetricsBuilder> metrics, int count = 1, [CallerArgumentExpression(nameof(metrics))] string? name = null) =>
        New(out metrics, CreateMetrics, count, name);
    ...
    protected virtual IMetricsBuilder CreateMetrics() => Substitute.For<IMetricsBuilder>();
