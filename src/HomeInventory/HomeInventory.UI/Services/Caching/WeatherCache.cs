...
namespace HomeInventory.UI.Services.Caching;
...
public sealed class WeatherCache : IWeatherCache
{
    ...
    {
        using var scope = _logger.BeginScope(new { Method = nameof(GetForecast) });
    ...
    private static async ValueTask<string?> GetCachedWeather(CancellationToken token)
    {
